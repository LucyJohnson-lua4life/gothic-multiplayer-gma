...
function VLK_2002_Jock()
	local npc = CreateNPC(GetNewNPCName("Jock"));
	
	SetPlayerAdditionalVisual(npc,"Hum_Body_Naked0",4, "Hum_Head_Fighter", 40);
	...
	local npcarr = {};
	npcarr.id = npc;
	npcarr.daily_routine = TA_VLK_2002;
...
function TA_VLK_2002(_playerid)
	
	if(TA_FUNC(_playerid, 0, 0, 24, 0))then
		AI_ClearStates(_playerid);
		AI_SETWALKTYPE(_playerid, 0);--Let him walk!
		AI_GOTOFP(_playerid, "FP_CITY_TAVERN_BARKEEPER");
		AI_ALIGNTOFP(_playerid, "FP_CITY_TAVERN_BARKEEPER");

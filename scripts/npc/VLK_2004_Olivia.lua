...
function VLK_2004_Olivia()
	local npc = CreateNPC(GetNewNPCName("Olivia"));
	
	SetPlayerAdditionalVisual(npc,"Hum_Body_Babe0",4, "Hum_Head_Babe", 154);
	...
	local npcarr = {};
	npcarr.id = npc;
	npcarr.daily_routine = TA_VLK_2004;
...
function TA_VLK_2004(_playerid)
	
	if(TA_FUNC(_playerid, 0, 0, 24, 0))then
		AI_ClearStates(_playerid);
		AI_SETWALKTYPE(_playerid, 0);--Let him walk!
		AI_GOTOFP(_playerid, "FP_HAFEN_VENDOR_MERCHANT");
		AI_ALIGNTOFP(_playerid, "FP_HAFEN_VENDOR_MERCHANT");
